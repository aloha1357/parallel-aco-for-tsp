...
class ComprehensiveACOExperiment {
private:
...
public:
    ...
    void runFixedIterationExperiment() {
        ...
        std::cout << "Starting Fixed Iteration Experiment..." << std::endl;
                ...
                for (int threads : thread_counts) {
                    for (int round = 1; round <= rounds; round++) {
                        try {
                            ...
                            if (round % 10 == 0) {
                                std::cout << "Progress: " << config.instance_name 
                                         << " iter=" << iterations 
                                         << " threads=" << threads 
                                         << " round=" << round << "/" << rounds << std::endl;
                            }
                        } catch (const std::exception& e) {
                            std::cerr << "Error in experiment: " << e.what() << std::endl;
                        }
                    }
                }
            }
        ...
        std::cout << "Fixed Iteration Experiment completed!" << std::endl;
    }
    ...
    void runTimeBudgetExperiment() {
        ...
        std::cout << "Starting Time Budget Experiment..." << std::endl;
        
        // First, measure 1-thread 100-iteration time for each instance
        std::vector<double> base_times;
        for (const auto& config : test_instances) {
            auto result = runSingleExperiment(config, 1, 100, 1);
            base_times.push_back(result.execution_time_ms);
            std::cout << "Base time for " << config.instance_name << ": " << result.execution_time_ms << "ms" << std::endl;
        }
            ...
            for (int threads : thread_counts) {
                for (int round = 1; round <= rounds; round++) {
                    try {
                        ...
                        if (round % 10 == 0) {
                            std::cout << "Progress: " << config.instance_name 
                                     << " budget=" << (int)time_budget << "ms"
                                     << " threads=" << threads 
                                     << " round=" << round << "/" << rounds << std::endl;
                        }
                    } catch (const std::exception& e) {
                        std::cerr << "Error in time budget experiment: " << e.what() << std::endl;
                    }
                }
            }
        }
        ...
        std::cout << "Time Budget Experiment completed!" << std::endl;
    }
};

int main() {
    try {
        ComprehensiveACOExperiment experiment;
        
        std::cout << "=== Comprehensive ACO Performance Experiment ===" << std::endl;
        std::cout << "Testing instances: eil51, kroA100, ch150, gr202" << std::endl;
