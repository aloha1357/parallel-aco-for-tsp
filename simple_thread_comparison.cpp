...
public:
    SimpleThreadComparator() : num_runs_per_test_(3) {
        // 设置测试地图 - 使用完整路径
        map_files_ = {
        ...
        base_params_.max_iterations = 50; // 减少迭代次数以加快测试
        base_params_.random_seed = 42;
        base_params_.enable_early_stopping = false;
    }
    
    std::vector<ThreadTestResult> runComparison() {
        ...
        for (const auto& map_file : map_files_) {
            ...
            try {
                // 加载地图
                auto graph_unique = aco::TSPLibReader::loadGraphFromTSPLib(map_file);
                if (!graph_unique) {
                    std::cerr << "  错误: 无法加载地图 " << map_file << std::endl;
                    continue;
                }
...
int main() {
    try {
        SimpleThreadComparator comparator;
        
        // 运行比较测试
        std::cout << "开始线程性能比较测试..." << std::endl;
        auto results = comparator.runComparison();
